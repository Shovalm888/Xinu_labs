...
struct	qent	q[NEWNQENT];	/* q table (see queue.c)		*/
...
//**LAB_6 - variables initialization

int father[NPROC], sons[NPROC], waitingPids[NPROC], semphs[NPROC];
slpMem slpMemtab[NUM_OF_SLPMEMS];
...
LOCAL sysinit()
{
	...
	//**LAB_6 - initialization:

	for (i = 0; i < NPROC; i++)
		semphs[i] = screate(0);
	for (i = 0; i < NUM_OF_SLPMEMS; i++)
		slpMemtab[i].ptr = NULL;
	meminsert_mutex = screate(1);
